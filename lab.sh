...
for ((i=0; i<${#batch[@]}; i++)); do
    # python tf_cnn_benchmarks.py --num_gpus=1 --model=$model --variable_update=parameter_server  --batch_size=${batch[$i]} > report/app/raw-data/$model-training/gpu-$model-b${batch[$i]}.txt
    python tf_cnn_benchmarks.py --device=gpu --model=$model --variable_update=parameter_server  --batch_size=${batch[$i]} > app/raw-data/$model-training/gpu-$model-b${batch[$i]}.txt
    wait
